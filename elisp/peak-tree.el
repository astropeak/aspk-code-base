...
(defun aspk/tree-iterate (tree fn &optional parent depth)
  "Iterate all element in the TREE, with function FN. The element, its parent subtree, and the element's depteh will be passed to the FN. PARENT is the parent sub tree of (car tree)"
  (or depth (setq depth 0))
  (when tree
    (funcall fn (car tree) parent depth)
    ;; -------elem can be seen by fn, so be becauful. This is a problem of dynamic scoping.
    (mapc (lambda (-------elem)
            (aspk/tree-iterate -------elem fn tree (+ depth 1)))
          (cdr tree))))
